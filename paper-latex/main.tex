...
更直接的成对思路是度量等值面相似性。Bruckner 和 Möller 的等值面相似性图（ISM）用归一化互信息比较等值面结构、选出代表性等值面，说明拓扑不变时几何仍可能改变。\cite{bruckner_ism_2010} 但 ISM 把局部结构编码为单一距离场再做整体比较：当连通分支发生生成、合并或消亡时，多个原本独立的分支被混入同一距离场，连通性改变前后的距离场仍然接近、区分度不足。此外，若在同一阈值下抽取全局等值面，还会混入属于其他分支的无关结构。这两点表明，几何驱动的度量需要被约束在合适范围内、并能够按连通分支分别组织。本文据此把 ISM 重新组织为按分支构造距离场、嵌入到多分支加权框架内的 per-branch 实例（\ref{sec:ism} 节）；这一改造使原版 ISM 的两点不足在外层框架下得到修复，本身亦构成本文的贡献之一。
...
\textbf{图表示学习与节点筛选。}除显式几何度量外，局部结构也可由数据学习其表示：GCN、GraphSAGE 能把邻域结构与节点特征编码为低维向量，GIN 刻画了消息传递网络区分图结构的能力。\cite{kipf_gcn_2017,hamilton_graphsage_2017,xu_gin_2019} 这为本文用图级 embedding 比较局部结构提供了工具——具体而言，把每个候选节点诱导的"活性子图"（原始网格上被等值面截穿的边及其端点）经 GCN 编码后比较，自然给出与 IoU/ISM 并列的第三种 BranchDiff 实例（详见 \ref{sec:gnn} 节）。

\subsection{研究动机}

上述梳理显示，拓扑驱动的简化与几何驱动的度量各有所长亦各有盲区：前者擅长去噪，但要么改删拓扑、要么仅凭连通性折叠链内普通节点；后者能刻画形状与规模，却把多个连通分支混为一谈、对连通性变化不敏感。本文的立场是融合二者：在严格保留拓扑结构的前提下，用连通分支感知的几何/结构差异来筛选链内普通节点。

这一立场具体落实为三项设计。其一，简化范围受拓扑约束：结构节点全部保留，候选普通节点只在所属拓扑链或局部区域内比较，简化后仍能重建合法树——这为拓扑侧顶点归约所缺的“连通性之外的保留判据”给出答案。其二，差异度量按连通分支分别组织：不同于 ISM 把整体距离场作单一比较，本文逐一比较各连通分支，使度量对分支的生成、合并与消亡保持敏感。其三，多分支场景下按重要性加权：一个标量层级可能同时穿过主分支与旁支，简单平均会压低小而重要的分支、纯按规模加权又会让主分支主导，故本文由拓扑树给出分支身份，再按拓扑显著性与几何规模加权汇总。

据此，本文将增广合并树普通节点简化表述为拓扑约束下的局部代表点选择问题。给定拓扑链 $v_0,v_1,\ldots,v_k$（$v_0$、$v_k$ 为必须保留的结构节点，中间为候选普通节点），算法从 $v_0$ 出发维护最近保留节点 $v_r$，依次比较候选 $v_i$ 与 $v_r$ 的局部差异 $D(v_r,v_i)$，超过阈值则保留并更新参考节点，最终保留 $v_k$。整体框架保持一致，方法间的差别只在局部分支差异 $d_i$ 的定义上。

\subsection{论文结构}

本文余下内容组织如下：第三节给出基于拓扑-几何约束的增广合并树普通节点简化方法，先介绍连通分支感知的多分支加权简化框架，再说明如何以局部活性子图的 GNN 向量化距离作为每个分支的基础差异度量，并给出多分支权重设计；第四节介绍实验数据、参数设置、对比基线（包括 ISM、体积 IoU 和 ActiveGraph）和简化结果分析，重点比较不同度量对保留节点数量、等值结构表达能力和连通分支变化识别能力的影响；第五节总结全文，并讨论进一步改进学习型结构编码、多分支权重和大规模数据处理效率的可能方向。

\section{方法}

本节先给出整体简化框架，说明本文如何在拓扑约束下把普通节点筛选组织成一个连通分支感知的链上贪心过程；随后给出关键细节，即如何用局部活性子图的 GNN 向量化定义单个分支上的差异 $d_\ell$，以及如何按拓扑显著性和几何规模把多个分支的差异加权汇总为整体差异 $D$。本文方法可概括为：以多分支加权为外层框架，以 GNN 活性子图距离替换原有的体积 IoU 作为每个分支的基础差异度量。
...
\end{equation}
并且在每条拓扑链 $C=(v_0,\ldots,v_k)$ 内，把 $K\cap C$ 视为按标量排序的序列 $v_0=u_0,u_1,\ldots,u_m=v_k$ 时，相邻保留节点之间的差异均不超过 $\varepsilon$：
\begin{equation}
D(u_{j-1},u_j)<\varepsilon,\quad j=1,\ldots,m,
\label{eq:eps-cover}
\end{equation}
在此约束下使 $|K|$ 最小。式~\eqref{eq:eps-cover}意味着每个被删除节点都落在某一对相邻保留节点的 $\varepsilon$-容差带内，因而简化误差沿链由 $\varepsilon$ 严格控制。沿链贪心扫描——总以最近一次保留节点 $v_r$ 为参考，遇到首个使 $D(v_r,v_i)\geq\varepsilon$ 的候选节点即保留并更新 $v_r$——可证明给出上述约束下的最小保留集，故本文采用贪心作为构造性解。
...
\paragraph{术语约定。}本文将"分支 (branch)"与"拓扑链 (chain)"视为同一对象的两个名字：以两个结构节点为端点、内部全为度数 $2$ 的普通节点的极大单调路径。具体地，分支 $b_\ell\subseteq V$ 是满足以下条件的节点集合：(i) 由两个端点结构节点和它们之间的一段普通节点序列构成；(ii) 按标量值排序后单调；(iii) 整棵增广合并树可无重叠地分解为分支集合 $\mathcal{B}=\{b_1,b_2,\ldots\}$。分支的标量范围记为 $[f_{\min}(b_\ell),f_{\max}(b_\ell)]$，定义为分支内全部节点标量的最小/最大值；分支的持久性记为 $\pi_\ell=f_{\max}(b_\ell)-f_{\min}(b_\ell)$。这一约定与文献中常见的 branch decomposition 一致：叶分支（leaf-to-saddle）、鞍-鞍分支（saddle-to-saddle）以及通向根的分支被同等看待，本文不预设主分支与旁支的区分。在算法叙述中我们交替使用两个名字：强调"框架沿链贪心扫描"时用拓扑链 $C=(v_0,\ldots,v_k)$，强调"按连通分支组织差异度量"时用分支 $b_\ell$，二者指同一对象。
...
\label{eq:aggregate}
\end{equation}
其中权重 $w_\ell$ 由分支的拓扑显著性（持久性 $\pi_\ell$）和几何规模 $s_\ell$ 通过混合系数 $\alpha$ 共同决定，具体形式见 \ref{sec:weight} 节式~\eqref{eq:wtopo}、\eqref{eq:wgeo}、\eqref{eq:wmix}（为保持本节叙述紧凑，权重设计放在三种 BranchDiff 实例之后单独讨论）。这一加权设计避免把所有分支简单平均、也避免大分支完全压制小而拓扑重要的分支。
...
\end{equation}
则保留 $v_i$ 并更新 $v_r\leftarrow v_i$，否则跳过；扫描结束后始终保留终点 $v_k$。这一策略的含义是：参考节点始终是上一个被认为“足够不同”的节点，因此被删除的连续普通节点都落在与同一参考节点差异不超过 $\varepsilon$ 的容差带内，保证简化误差沿链可控而不会逐点累积漂移。对全部拓扑链完成保点判定后，再根据保留节点集合合并被删除节点跨越的弧——新弧的标量跨度取两端保留节点之差，区域规模取被合并各弧之和——从而重建出仍然合法的简化合并树。
...
图~\ref{fig:retention} 以一个三分支同时活跃的例子，示意多分支加权差异如何影响保留判定：小分支 $b_2$ 几何规模虽小却正经历剧烈截交变化（示意值 $d_2=0.75$），而大分支 $b_1$ 几乎不变；若按纯几何规模加权，$b_2$ 被大分支淹没，汇总差异 $D=0.22<\varepsilon$，$v_i$ 被误删；本文以拓扑显著性与几何规模共同加权（式~\eqref{eq:wmix}），令持久性高的小分支获得足够权重，$D=0.38\geq\varepsilon$ 从而正确保留 $v_i$。
...
第二种实例化关注分支局部等值面的几何形态，用信息论度量比较两个节点诱导的距离场，作为纯几何对照。它能区分体积相近而形状不同的结构，但仍把局部结构压缩为单一距离场，不显式区分连通分支。
...
第三种实例化关注分支局部等值面在原始网格上的连通结构。前两种度量分别只刻画规模与距离场形态，对连通分支的生成、合并、消亡不敏感；本节将分支差异定义为原始网格活性子图的 GNN 向量化距离，使其能反映等值面截交结构的变化，并在连通分支改变时给出显著响应。
...
\label{eq:gcn}
\end{equation}
其中 $\hat{A}=\tilde{D}^{-1/2}\tilde{A}\,\tilde{D}^{-1/2}$ 为加自环后的对称归一化邻接矩阵，$W^{(1)}\in\mathbb{R}^{11\times h}$、$W^{(2)}\in\mathbb{R}^{h\times h}$ 为可学习权重，$h$ 为隐藏维度。两层传播使每个顶点聚合其二跳邻域信息，足以刻画活性子图的局部拓扑。随后对顶点表示 $H^{(2)}$ 做排列不变的读出（readout），拼接均值池化与最大值池化结果，并附加两个对数规模项以保留子图大小信息：
\begin{equation}
\begin{aligned}
...
\paragraph{权重来源。}GCN 权重 $W^{(1)},W^{(2)}$ 有两种取法。其一是确定性随机初始化：由固定随机种子按 He 初始化生成权重，不经训练，此时 GCN 退化为一个固定的、排列不变的图特征提取器，其区分能力来自结构与特征本身。其二是对比学习：以同一活性子图的两次随机增强（如顶点特征扰动、邻域子采样）为正样本对、同一批次内其他子图为负样本，用 InfoNCE / NT-Xent 目标
\begin{equation}
\mathcal{L}=-\log\frac{\exp\bigl(\cos(z,z^{+})/\tau\bigr)}{\sum_{z^{-}}\exp\bigl(\cos(z,z^{-})/\tau\bigr)},
\end{equation}
训练 GCN，使结构相似的子图在嵌入空间中靠近、不相似者远离，$\tau$ 为温度系数。两种取法均可作为本框架的合法 GNN backend：前者不依赖标注、适合零样本场景，后者在有充足数据时能进一步提升对结构变化的区分度。本文方法章节给出二者的统一形式化，第四节将在两类权重下分别给出实验结果。

\paragraph{分支差异。}参考节点与候选节点在分支 $b_\ell$ 上的差异由两图级向量的余弦距离给出：
\begin{equation}
d_\ell(v_r,v_i)=1-\cos\bigl(z_\ell(v_r),\,z_\ell(v_i)\bigr).
\end{equation}
当两个局部活性子图结构高度相似时余弦接近 $1$、$d_\ell$ 接近 $0$；当截交结构发生明显变化（包括连通分支的生成、合并或消亡导致活性边集合的拓扑重组）时 $d_\ell$ 增大。相比把多个分支混入单一整体描述符的纯几何度量，这一逐分支定义使差异对连通性变化保持敏感。对应的几何规模权重取活性边数 $|E_{\mathrm{act}}|$。算法~\ref{alg:gnn} 给出其计算过程。
...
拓扑显著性权重由分支持久性给出。分支 $b_\ell$ 的持久性取其标量跨度 $\pi_\ell=f_{\max}(b_\ell)-f_{\min}(b_\ell)$，归一化后
\begin{equation}
w^{\mathrm{topo}}_\ell=\frac{\pi_\ell}{\sum_{k}\pi_k},
\label{eq:wtopo}
\end{equation}
使持久性大的主分支获得更高权重。几何规模权重由分支局部结构的规模 $s_\ell$ 给出并归一化
\begin{equation}
w^{\mathrm{geo}}_\ell=\frac{s_\ell}{\sum_{k}s_k},
\label{eq:wgeo}
\end{equation}
其中规模 $s_\ell$ 由所采用的差异度量提供——IoU 取体积集合大小 $|\mathcal{V}_\ell|$，ISM 取局部块边界体素数，GNN 取活性边数 $|E_{\mathrm{act}}|$——使局部结构规模大的分支获得更高权重。二者通过混合系数 $\alpha\in[0,1]$ 线性组合并归一化：
\begin{equation}
...
两类权重的互补性是这一设计的关键。纯拓扑权重 $w^{\mathrm{topo}}$ 倾向于保护持久性大的主分支，但当某条旁支虽然持久性不高、却在当前层级正经历剧烈的截交结构变化时，单靠拓扑权重会低估它；纯几何规模权重 $w^{\mathrm{geo}}$ 能捕捉到这种规模变化，但若只用它，大分支会凭借体量持续主导、淹没小而拓扑重要的分支。通过 $\alpha$ 线性混合，可在“保护重要拓扑分支”与“响应剧烈几何变化”之间取得平衡。$\alpha$ 的合适取值与数据特性有关：分支持久性差异显著、希望严格保主干时取较大 $\alpha$；分支尺度差异大、关注局部几何演化时取较小 $\alpha$。本文在实验中考察 $\alpha$ 对保留节点数与等值结构表达能力的影响。
...
\end{itemize}
三种度量量级差异显著（IoU 最轻、ISM/GNN 重一到两个数量级），但都把局部结构严格限制在分支包围盒内，因而 $\bar{s}$ 与局部规模相关而与整场体素总数无关，使方法在大体数据上仍保持可控的单步开销；实际运行时间主要由活跃分支数和分支局部规模决定，而非全树节点数。此外，参考节点的局部描述符（体积集合、距离场或子图嵌入）可在贪心推进时缓存复用，避免重复计算同一参考节点。
