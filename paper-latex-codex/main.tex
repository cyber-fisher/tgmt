...
\textbf{轮廓树与持久性。}科学计算、医学影像和流体模拟中的数据常表示为定义在空间域上的标量场 $f:\Omega\rightarrow\mathbb{R}$。Marching Cubes 等方法可显式重建单一阈值的等值面，但逐阈值抽取难以揭示结构随阈值变化的全局层次。\cite{lorensen_cline_1987} 拓扑数据分析为此提供了更紧凑的表示：轮廓树通过刻画等值面连通分量随阈值变化的演化，把连续阈值空间压缩为树结构。\cite{van_kreveld_1997,carr_contour_2003} 具体而言，轮廓树记录等值面 $f^{-1}(\alpha)$ 的连通分量随阈值 $\alpha$ 变化而出生、合并、分裂与消亡的过程；它可由记录子水平集 $F_\alpha=\{x\in\Omega\mid f(x)\leq\alpha\}$ 连通分量演化的并树（join tree）与记录超水平集演化的分裂树（split tree）合并得到。极值点、鞍点和分支节点对应关键拓扑事件，树边表示相邻事件之间的演化区间；持久性则用于衡量特征显著性，短寿命特征常被视为噪声。\cite{edelsbrunner_persistence_2002} Topology ToolKit（TTK）将这些结构封装为可与 ParaView、VTK 衔接的工程模块，使输入是带有节点标量、临界类型、坐标和弧分割区域的拓扑数据结构。\cite{tierny_ttk_2018}

\textbf{增广轮廓树的冗余问题。}传统轮廓树只保留临界节点，不足以支撑交互探索与等值面追踪——两个相邻拓扑事件之间虽无连通性变化，等值面仍可能明显改变形状、体积或穿过新的网格区域。为保留这些中间状态，增广轮廓树在临界节点之间插入大量普通节点，并将树边关联到原始数据的分割区域。\cite{gueunet_act_2019,gueunet_forests_2016,li_distributed_2024} 但增广也带来冗余：普通节点过密使树难以阅读，相邻普通节点的局部等值结构往往高度相似，全部保留会产生近似重复的等值面并增加存储与交互成本。由此引出本文的核心问题：在保留关键拓扑结构的前提下，如何筛选出真正有代表性的普通节点。围绕这一问题，已有工作可沿两个方向梳理——以删除拓扑特征为目标的拓扑驱动简化，以及以几何量衡量结构差异的几何驱动度量；本文方法正是为弥合二者的不足而提出。
...
然而这两类操作都不契合本文目标。叶子剪枝会改删拓扑，而本文要求严格保留根、叶、鞍点和分支节点；顶点归约虽不改标量场，但其判据纯粹是连通性，一旦套用到增广轮廓树，整条拓扑链上的普通节点都会被无差别折叠成一条超弧，链内记录的等值面几何演化随之全部丢失。换言之，拓扑驱动的简化要么改删拓扑、要么仅凭连通性折叠链内节点，缺少一个在保留拓扑结构的前提下、按几何代表性筛选链内普通节点的判据。
...
更直接的成对思路是度量等值面相似性。Bruckner 和 Möller 的等值面相似性图（ISM）用归一化互信息比较等值面的距离场结构、选出代表性等值面，说明拓扑不变时几何仍可能改变。\cite{bruckner_ism_2010} 但这类方法把一个标量层级上的等值结构编码为单一距离场：一方面，多个连通分支的变化会被混合到同一个描述符中；另一方面，若在同一阈值下抽取全局等值面，还可能引入属于其他分支的无关结构。更根本地，距离场是对等值面几何分布的连续刻画，并不显式保留它在原始离散网格上截穿了哪些边——而后者恰好提供了一种更贴近底层数据、且天然带连通关系的局部结构描述。这启示本文：成对的几何/结构度量既要被限制在合适的拓扑范围内、在多分支同时活跃时保留分支身份，也宜建立在原始网格的离散结构之上。本文据此不再使用整体距离场，而是以分支局部的活性子图（原始网格上被等值面截穿的边及其端点）作为统一的结构描述子（\ref{sec:subgraph} 节）。

\subsection{轮廓树比较与图表示学习}

与逐数据集内部简化相关的另一脉络是树之间的比较：结构平均、编辑距离、Wasserstein 距离和 interleaving distance 等被用于比较不同标量场、时间步或 ensemble 成员。\cite{yan_average_2020,pont_wasserstein_2022,wetzels_branch_2022} 这些方法多在合并树上提出，但轮廓树可由并树与分裂树合并得到，其局部比较思想同样适用。其中局部比较思想尤其相关——Sridharamurthy 和 Natarajan 的局部树编辑距离强调细粒度的局部比较，Yan 等的 geometry-aware 比较把几何信息引入树间对应。\cite{sridharamurthy_local_2023,yan_geometry_2023} 它们提示普通节点筛选应结合局部拓扑位置与局部几何，但目标是度量两棵树之间的差异，而本文是在同一棵树内部沿拓扑链筛选冗余节点，无需求树间对应。

\textbf{图表示学习与节点筛选。}除显式几何度量外，局部结构也可由数据学习其表示：GCN、GraphSAGE 能把邻域结构与节点特征编码为低维向量，GIN 刻画了消息传递网络区分图结构的能力。\cite{kipf_gcn_2017,hamilton_graphsage_2017,xu_gin_2019} 这为本文用图级 embedding 比较局部结构提供了工具——具体而言，把每个候选节点诱导的"活性子图"（原始网格上被等值面截穿的边及其端点）经 GCN 编码后比较，可作为统一多分支框架下的第二种 \textsc{BranchDiff} 实例（详见 \ref{sec:gnn} 节）。

\subsection{研究动机}

上述梳理显示，拓扑驱动的简化与几何驱动的度量各有所长亦各有盲区：前者擅长去噪，但要么改删拓扑、要么仅凭连通性折叠链内普通节点；后者能刻画形状与规模，但若脱离轮廓树分支来整体比较，可能混合多个连通分支的变化。本文的核心立场是融合二者：在严格保留拓扑结构的前提下，提出一个拓扑保持的多分支感知增广轮廓树节点简化框架，并在该框架内用分支级几何/结构差异筛选链内普通节点。

这一框架具体落实为三项设计。其一，简化范围受拓扑约束：结构节点全部保留，候选普通节点沿所属拓扑链顺序筛选，简化后通过合并原始弧重建合法树。其二，基础差异度量按分支实例化：本文统一以原始网格上的局部活性子图刻画分支局部结构，并给出两种比较方式——活性边集合的 Jaccard 距离与活性子图的 GNN 向量化距离，分别从显式边集和向量化结构两个角度定义 \textsc{BranchDiff}，但都输出统一的单分支差异 $d_\ell$。其三，多分支场景下按重要性加权：一个标量层级可能同时穿过主分支与旁支，本文保留这些分支身份，并按标量跨度型拓扑显著性与几何规模把多个 $d_\ell$ 汇总为整体差异 $D$。

据此，本文将增广轮廓树普通节点简化表述为拓扑约束下的局部代表点选择问题。给定拓扑链 $v_0,v_1,\ldots,v_k$（$v_0$、$v_k$ 为必须保留的结构节点，中间为候选普通节点），算法从 $v_0$ 出发维护最近保留节点 $v_r$，依次比较候选 $v_i$ 与 $v_r$ 的多分支加权差异 $D(v_r,v_i)$，超过阈值则保留并更新参考节点，最终保留 $v_k$。因此，本文的主线不是提出彼此独立的多个简化器，而是给出一个固定的拓扑保持框架；活性边集合 Jaccard 与活性子图 GNN 向量化只替换其中的分支差异子过程。

\subsection{论文结构}

本文余下内容将围绕这一框架展开：方法部分先介绍拓扑链分解、链上筛选和多分支加权汇总，再给出局部活性子图及其两种 \textsc{BranchDiff} 实例——边集 Jaccard 与 GNN 向量化。后续实验部分可据此比较不同实例和参数设置对节点保留数量、局部结构表达与分支变化响应的影响。

\section{方法}

本节先给出整体简化框架，说明本文如何在拓扑约束下把普通节点筛选组织成一个连通分支感知的链上贪心过程；随后给出关键细节，即如何定义单个分支上的差异 $d_\ell$，以及如何按拓扑显著性和几何规模把多个分支的差异加权汇总为整体差异 $D$。本文方法可概括为：以拓扑保持的多分支加权为外层框架，以原始网格上的局部活性子图为统一结构描述子，并以其边集 Jaccard 距离和 GNN 向量化距离作为两个可替换的 \textsc{BranchDiff} 实例。

\subsection{问题定义}

给定定义在规则网格 $\Omega$ 上的标量场 $f:\Omega\rightarrow\mathbb{R}$ 及其增广轮廓树 $T=(V,E,f)$。节点集合 $V$ 可按拓扑角色分为两类：结构节点集合 $V_s$，包括根节点、叶节点（局部极值）和分支节点（鞍点），它们刻画连通性发生改变的关键拓扑事件；普通节点集合 $V_o=V\setminus V_s$，即度数为 $2$、不改变连通性的增广节点。增广带来的冗余主要集中在 $V_o$：相邻普通节点对应的局部等值结构往往高度相似，却被完整保留。
...
\label{eq:eps-cover}
\end{equation}
该约束把链内节点筛选转化为代表点选择问题：保留节点不必覆盖所有细节，但相邻代表节点之间的局部差异被限制在预设容差内。本文采用线性、可控且易实现的沿链贪心策略作为构造性解：总以最近一次保留节点 $v_r$ 为参考，遇到首个使 $D(v_r,v_i)\geq\varepsilon$ 的候选节点即保留并更新 $v_r$。这一策略不依赖全局优化，适合在增广轮廓树的多条拓扑链上独立执行。

由 $K$ 重建简化轮廓树 $T'$ 的规则如下：对每条拓扑链按 $K\cap C$ 的相邻保留对 $(u_{j-1},u_j)$ 生成一条新弧，其标量跨度取 $|f(u_j)-f(u_{j-1})|$，区域规模 $\text{RegionSize}$ 取该对在原树中所跨越的全部原始弧的累加；端点连通关系由原拓扑链端点继承，从而 $T'$ 与 $T$ 保持相同的根、叶与分支结构，是一棵合法轮廓树。这一约束有两层含义：结构节点必须全部保留（拓扑结构决定哪些节点不能删），普通节点只在其所属拓扑链和局部连通分支内被比较和取舍（拓扑链决定哪些节点可比较）。问题的核心因此归结为：如何定义两个普通节点之间在局部连通分支上的差异 $D$，以及如何沿链选出一组能代表链内主要变化的普通节点。

\subsection{整体框架}

\paragraph{术语约定。}本文在算法层面区分两个相关概念。"拓扑链 (chain)" 指以两个结构节点为端点、内部全为度数 $2$ 普通节点的极大单调路径，是链上贪心扫描的基本单位。"分支 (branch)" 指在某一标量层级附近参与局部差异计算的链段或连通分量，用 $b_\ell$ 表示；在本文实现中，它由同一链段关联的分割标签和标量范围确定。为简化记号，本文常用分支 $b_\ell\subseteq V$ 表示一条满足以下条件的节点序列：(i) 由两个端点结构节点和它们之间的一段普通节点构成；(ii) 按标量值排序后单调；(iii) 各链段对整棵增广轮廓树形成无重叠分解。该工作定义服务于本文的节点筛选算法，不声称与所有文献中的 branch decomposition 完全等同。分支的标量范围记为 $[f_{\min}(b_\ell),f_{\max}(b_\ell)]$，定义为分支内全部节点标量的最小/最大值；其标量跨度型拓扑显著性记为 $\pi_\ell=f_{\max}(b_\ell)-f_{\min}(b_\ell)$，作为持久性思想的工程近似。在算法叙述中，强调扫描顺序时使用拓扑链 $C=(v_0,\ldots,v_k)$，强调局部差异和权重时使用分支 $b_\ell$。
...
% ---- node coordinates ----
\node[stage] (input) {增广\\轮廓树\\$T=(V,E,f)$};
...
\node[draw, rounded corners=1.5pt, fill=orange!12, font=\scriptsize, align=center,
      minimum width=16mm, minimum height=8mm, below=7mm of diff.south, xshift=-10mm] (jac)
      {Jaccard\\\scriptsize$1{-}J$};
\node[draw, rounded corners=1.5pt, fill=orange!12, font=\scriptsize, align=center,
      minimum width=16mm, minimum height=8mm, below=7mm of diff.south, xshift=10mm] (gnn)
      {GNN\\\scriptsize$1{-}\cos$};
\draw[->, line width=0.6pt, orange!70!black] (jac) -- (diff);
...
\end{tikzpicture}
\caption{本文方法总览：输入 $\to$ 分解 $\to$ 遍历 $\to$ 筛选 $\to$ 输出。BranchDiff 子过程在 Jaccard/GNN 两种实例间替换并输出 $d_\ell$，经多分支加权汇总为 $D$，再交链上贪心判定。}
\label{fig:overview}
\end{figure*}

\paragraph{树重建与结构节点识别。}算法首先从 txt 树结构（每行 \texttt{nodeId\ parentId\ scalar}）重建父子关系：以 \texttt{parentId} 指向自身或为负者为根，其余节点挂接到对应父节点，得到一棵有根树。每个节点的度数由其子节点数与父边共同决定，据此判定拓扑角色——根节点、叶节点（无子节点）和分支节点（子节点数大于一）归入结构节点 $V_s$，其余度数为 $2$ 的节点归入候选普通节点 $V_o$。树类型（Join Tree 或 Split Tree）决定标量的单调方向，进而决定后续活性边提取时的阈值方向。
...
\paragraph{连通分支感知的差异。}每条链上每个候选节点是否保留，不取决于其在全局等值面中的表现，而取决于它相对于最近保留参考节点 $v_r$ 的局部差异。与仅在当前链内比较不同，本文显式考虑该标量层级附近的多个连通分支：在轮廓树中，一个等值层级可能同时穿过多条分支，若只比较当前链自身或把所有分支混为一谈，都会丢失旁支上的真实变化。记每条分支 $b$ 的标量范围为 $[\,f_{\min}(b),f_{\max}(b)\,]$，对候选节点 $v_i$（阈值 $t_i=f(v_i)$），其活跃分支集合定义为该层级穿过的所有分支：
\begin{equation}
...
\label{eq:aggregate}
\end{equation}
其中权重 $w_\ell$ 由分支的标量跨度型拓扑显著性 $\pi_\ell$ 和几何规模 $s_\ell$ 通过混合系数 $\alpha$ 共同决定，具体形式见 \ref{sec:weight} 节式~\eqref{eq:wtopo}、\eqref{eq:wgeo}、\eqref{eq:wmix}（为保持本节叙述紧凑，权重设计放在两种 \textsc{BranchDiff} 实例之后单独讨论）。这一加权设计避免把所有分支简单平均、也避免大分支完全压制小而拓扑重要的分支。
...
\end{equation}
则保留 $v_i$ 并更新 $v_r\leftarrow v_i$，否则跳过；扫描结束后始终保留终点 $v_k$。这一策略的含义是：参考节点始终是上一个被认为“足够不同”的节点，因此被跳过的连续普通节点都相对于同一参考节点保持在阈值以内，使链内代表点的间隔由 $\varepsilon$ 直接控制，而不是逐点累积漂移。对全部拓扑链完成保点判定后，再根据保留节点集合合并被删除节点跨越的弧——新弧的标量跨度取两端保留节点之差，区域规模取被合并各弧之和——从而重建出仍然合法的简化轮廓树。

整体流程如算法~\ref{alg:framework} 和图~\ref{fig:overview} 所示。框架的不变部分（链分解、活跃分支识别、多分支加权汇总、链上贪心、重建）对所有度量保持一致，所有拓扑链统一采用连通分支感知的多分支加权——其中权重 $w_\ell$ 由分支的拓扑显著性与几何规模共同决定，是本文 framework 级的核心贡献，与具体 BranchDiff 实例正交。唯一与具体度量相关的部分，是计算单个分支差异的子过程 $\textsc{BranchDiff}(v_r,v_i,b_\ell)$，它输出单分支差异 $d_\ell$ 后由式~\eqref{eq:aggregate} 汇总为整体差异 $D=\sum_\ell w_\ell\, d_\ell$。本文的两种实例都建立在同一个局部活性子图（\ref{sec:subgraph} 节）之上：活性边集合的 Jaccard 距离（\ref{sec:jaccard} 节，算法~\ref{alg:jaccard}）与局部活性子图的 GNN 向量化距离（\ref{sec:gnn} 节，算法~\ref{alg:gnn}）。前者直接比较等值面截穿的网格边集合，可解释性强；后者把同一子图编码成向量，对扰动更鲁棒；两者互为对照，依次从“显式边集”到“向量化结构”刻画分支差异。其中权重 $w^{\mathrm{geo}}_\ell$ 所需的分支局部规模由活性子图规模（活性边数）给出。
...
\label{alg:framework}
\begin{algorithmic}[1]
\Require 轮廓树 $T=(V,E,f)$，阈值 $\varepsilon$，混合系数 $\alpha$
...
\ForAll{拓扑链 $C=(v_0,\ldots,v_k) \in \mathcal{C}$}
  ...
  \For{$i \gets 1$ \textbf{to} $k-1$}
    ...
    \ForAll{$b_\ell \in \mathcal{B}(t_i)$}
      \State $d_\ell \gets \textsc{BranchDiff}(v_r,v_i,b_\ell)$ \Comment{算法~\ref{alg:jaccard}/\ref{alg:gnn}}
...
% ---------- Panel (a): augmented contour tree with level t* crossing 3 branches ----------
...
本文的两种 \textsc{BranchDiff} 实例都建立在同一个局部结构描述子之上：等值面在原始网格上截穿的边及其端点所构成的局部活性子图。本节先给出其定义，随后两小节分别用边集 Jaccard 距离和 GNN 向量化距离来比较两个节点的活性子图。

\paragraph{活性边与活性子图。}等值面 $\{x\mid f(x)=t\}$ 在规则网格上的离散痕迹，是那些被该等值面穿过的网格边。对原始网格边 $(p,q)$，若其两端点标量值跨越阈值 $t=f(v)$，即
\begin{equation}
...
\end{equation}
相比直接重建等值面三角网格，活性子图无需插值和三角化，且天然保留了原始网格的连通关系；它把"等值面扫过了哪些局部结构"这一问题转化为对一组网格边的比较。

\paragraph{分支局部化。}为把比较限制在当前分支、排除同一阈值下其他分支的无关活性边，遍历范围由分支 $b_\ell$ 关联弧的分割标签（\texttt{SegmentationId}）集合确定：仅在这些分割区域的联合包围盒内枚举网格边，并要求边的至少一个端点落在目标分割内。由此得到的 $G_\ell(v)$ 只反映 $b_\ell$ 上的等值面截交结构。两个节点 $v_r,v_i$ 在分支 $b_\ell$ 上的差异 $d_\ell(v_r,v_i)$，即归结为比较它们的活性子图 $G_\ell(v_r)$ 与 $G_\ell(v_i)$。本文给出由粗到细的两种比较方式：直接比较活性边集合的 Jaccard 距离（\ref{sec:jaccard} 节），以及把活性子图编码为向量后比较的 GNN 向量化距离（\ref{sec:gnn} 节）。

\subsection{活性边集合的 Jaccard 差异}
\label{sec:jaccard}

第一种实例化直接比较两个节点活性边集合的重合程度，是最直观、可解释的结构差异度量。由于所有活性子图都嵌在同一母网格上、共享一致的网格边编号，边集是否重合本身就具有明确含义：两个节点若截穿了几乎相同的网格边，说明等值面在该分支上扫过的局部结构基本未变；反之，活性边集合差异越大，说明等值面已经移动到新的网格区域或局部截交结构发生重组。

\paragraph{比较方式。}记 $E_{\mathrm{act}}(v_r)$、$E_{\mathrm{act}}(v_i)$ 为参考节点与候选节点在分支 $b_\ell$ 上的活性边集合，二者的 Jaccard 相似度与对应差异为
\begin{equation}
J=\frac{|E_{\mathrm{act}}(v_r)\cap E_{\mathrm{act}}(v_i)|}{|E_{\mathrm{act}}(v_r)\cup E_{\mathrm{act}}(v_i)|},\qquad
d^{J}_\ell=1-J.
\label{eq:jaccard}
\end{equation}
两个边集完全重合时 $J=1$、$d^{J}_\ell=0$；完全不相交时 $d^{J}_\ell=1$。然而纯边集 Jaccard 对等值面的轻微平移较敏感：当等值面整体小幅移动、截穿了一批位置相邻但编号不同的网格边时，边集交集会骤降，使 $d^{J}_\ell$ 偏大而高估真实变化。为缓解这一点，本文引入两个对平移与尺度更平滑的几何补偿项。设活性子图顶点集 $V_{\mathrm{act}}(v)$ 的质心为 $c(v)$、包围盒对角线长度为 $\mathrm{diag}(v)$，定义质心位移项与包围盒尺度变化项
\begin{align}
...
\label{eq:jbbox}
\end{align}
其中 $\epsilon_0$ 为防止除零的小正数。三项取最大值合成该分支上的单分支差异
\begin{equation}
...
\end{equation}
$\lambda_p,\lambda_b$ 为两个补偿项的相对权重。取最大值而非求和，使得任一通道（边集重组、整体平移、尺度膨胀）发生显著变化都足以触发保留，符合"只要等值面在结构、位置或规模任一方面明显改变就保留该节点"的直觉。该度量实现简单、阈值含义清晰，缺点是边集匹配按精确编号进行，对网格离散化和轻微扰动的鲁棒性弱于下一节的向量化方法。对应的几何规模权重取活性边数 $|E_{\mathrm{act}}|$。算法~\ref{alg:jaccard} 给出其计算过程。

\begin{algorithm}[H]
\caption{\textsc{BranchDiff} 的 Jaccard 实例}
\label{alg:jaccard}
\begin{algorithmic}[1]
...
\Function{BranchDiff}{$v_r,v_i,b_\ell$}
  \ForAll{$v \in \{v_r,\,v_i\}$}
    \State $t \gets f(v)$；由 $b_\ell$ 分割标签及包围盒确定区域 $\Omega$
    \State 按式~\eqref{eq:active} 在 $\Omega$ 内提取活性边集 $E_{\mathrm{act}}(v)$
    \State 算顶点质心 $c(v)$ 与包围盒对角线 $\mathrm{diag}(v)$
  \EndFor
  \State 由式~\eqref{eq:jaccard} 算边集 Jaccard 距离 $d^{J}_\ell$
  \State 由式~\eqref{eq:jpos}、\eqref{eq:jbbox} 算 $d^{\mathrm{pos}}_\ell,d^{\mathrm{bbox}}_\ell$
  \State \Return $\max\bigl(d^{J}_\ell,\lambda_p\,d^{\mathrm{pos}}_\ell,\lambda_b\,d^{\mathrm{bbox}}_\ell\bigr)$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
边集 Jaccard 直接、可解释，但它只看"边是否完全重合"，对网格离散化、轻微平移和少量边的增删较敏感：两个局部结构整体相似，却可能因为少数边编号不同而 Jaccard 距离骤升。第二种实例化为此把同一个局部活性子图编码成固定长度向量，在向量空间中比较结构相似性，从而把"边是否逐一重合"放松为"局部结构是否相似"，对扰动更鲁棒，也能把标量、空间位置与分割信息一并编码进统一描述子。

\paragraph{节点特征。}活性子图刻画的是局部结构，因此顶点特征应同时编码标量、空间与分割信息，并对全局平移、尺度保持稳定。对每个活性顶点 $p$，构造 $11$ 维特征向量 $x_p$：第 $1$ 维为全局归一化标量 $(f(p)-f_{\min})/(f_{\max}-f_{\min})$；第 $2$、$3$ 维为相对阈值的有符号偏差 $(f(p)-t)$ 及其绝对值（均按标量范围归一化），用于区分顶点位于等值面内侧还是外侧；第 $4$ 至 $6$ 维为顶点相对子图质心的坐标，按子图包围盒对角线长度归一化，从而对平移和尺度不变；第 $7$ 维为按最大网格度数（$6$）归一化的顶点度数；第 $8$ 至 $11$ 维为分割标签的多尺度正余弦编码，使不同分割归属可被区分而又不引入人为的大小顺序。这些特征共同保证：整体平移或缩放数据不改变向量，而局部连通结构、标量分布或分割归属的变化会改变向量。
...
\label{eq:readout}
\end{equation}
其中 $z_\ell(v)\in\mathbb{R}^{2h+2}$，最后经 $L_2$ 归一化，使后续余弦比较只关注向量方向。均值池化概括子图的整体分布，最大池化捕捉最突出的局部响应，两个对数规模项则补回池化所丢失的子图绝对大小信息——否则一大一小但分布相近的两个子图可能被误判为相同。

\paragraph{权重来源。}为使 GNN 实例不依赖额外标注，本文默认采用确定性固定权重：由固定随机种子按 He 初始化生成 $W^{(1)},W^{(2)}$，训练和测试阶段均保持不变。此时 GCN 可视为一个固定的、排列不变的图结构特征提取器，其差异来自活性子图的邻接关系与节点特征本身，而非学习到的参数。这一选择保证方法在无标注数据上即可运行，并使结果可复现。若后续具备更充分的数据，也可以把同一结构扩展为对比学习编码器：以同一活性子图的两次随机增强（如顶点特征扰动、邻域子采样）为正样本对、同一批次内其他子图为负样本，用 InfoNCE / NT-Xent 目标
\begin{equation}
...
\label{eq:gnndist}
\end{equation}
当两个局部活性子图结构高度相似时余弦接近 $1$、$d_\ell$ 接近 $0$；当截交结构发生明显变化（例如活性边集合的连通关系重组）时 $d_\ell$ 通常增大。与上一节的边集 Jaccard 相比，向量化把离散的"边是否重合"转化为连续的"结构是否相似"，因而对轻微平移与少量边增删更鲁棒，代价是其相似度的可解释性弱于显式边集。对应的几何规模权重取活性边数 $|E_{\mathrm{act}}|$。算法~\ref{alg:gnn} 给出其计算过程。
...
\label{eq:wgeo}
\end{equation}
其中规模 $s_\ell$ 由分支局部活性子图的活性边数 $|E_{\mathrm{act}}|$ 给出——两种实例共享同一活性子图，故规模口径一致——使局部结构规模大的分支获得更高权重。二者通过混合系数 $\alpha\in[0,1]$ 线性组合并归一化：
\begin{equation}
...
设轮廓树节点数为 $|V|$、候选普通节点总数为 $|V_o|$、单个标量层级的活跃分支数为 $q$（通常远小于分支总数）。树重建、结构节点识别与拓扑链分解只需常数次遍历，复杂度为 $O(|V|)$。链上贪心对所有拓扑链的每个候选普通节点各处理一次（各链无重叠，合计 $|V_o|$ 个节点），每次在至多 $q$ 个活跃分支上计算分支差异 $\textsc{BranchDiff}$，因此节点筛选的总代价约为
\begin{equation}
O\bigl(|V_o|\,q\,C(\bar{s})\bigr),
\end{equation}
其中 $C(\bar{s})$ 为单分支差异的计算代价，依赖于具体度量。两种实例都先在分支局部区域内提取活性子图（设其活性边数为 $\bar{s}=|E_{\mathrm{act}}|$，通常 $O(10^2)$–$O(10^4)$），其后比较代价不同：
\begin{itemize}
\item \textbf{Jaccard} 实例：$C=O(\bar{s})$，主要为两个活性边集合的交并运算（边按编号哈希后线性求交并），外加 $O(\bar{s})$ 的质心与包围盒统计。计算最轻。
\item \textbf{GNN} 实例：$C=O(\bar{s}\,h)$，由两层 GCN 前向传播主导（每条活性边参与一次邻域聚合，$h$ 为隐藏维度），外加 $O(\bar{s})$ 的特征构造与池化读出。计算较重。
\end{itemize}
两种 \textsc{BranchDiff} 实例的代价量级不同（Jaccard 最轻、GNN 较重），但它们共享同一活性子图提取、都把局部结构限制在分支相关区域内，因而 $\bar{s}$ 主要由局部分支规模决定，而非由整场体素总数直接决定；实际运行时间还受活跃分支数与分支局部规模影响。此外，参考节点的局部描述符（活性边集合或子图嵌入）可在贪心推进时缓存复用，避免重复计算同一参考节点。
