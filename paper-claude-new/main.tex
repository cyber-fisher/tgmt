...
\paragraph{轮廓树。}拓扑数据分析为此提供了更紧凑的表示。轮廓树（contour tree）通过刻画等值面连通分量随阈值变化的演化，把连续阈值空间压缩为一棵树：它记录等值面 $f^{-1}(\alpha)$ 的连通分量随阈值 $\alpha$ 变化而出生、合并、分裂与消亡的过程。\cite{van_kreveld_1997,carr_contour_2003} 形式上，它可由记录子水平集 $F_\alpha=\{x\in\Omega\mid f(x)\leq\alpha\}$ 连通分量演化的并树（join tree）与记录超水平集演化的分裂树（split tree）合并得到。极值点、鞍点和分支节点对应关键拓扑事件，树边则表示相邻事件之间的演化区间。
...
\centering
\small
\caption{三维标量场中各类临界点（Morse 指数 $i$）对等值面欧拉示性数 $\chi$ 的贡献及其拓扑含义。$\chi$ 增减 $(-1)^i$，亏格由 $g=(2c-\chi)/2$ 反推。}
\label{tab:critical}
\begin{tabular}{cccl}
\hline
类型 & 指数 $i$ & $\Delta\chi$ & 拓扑事件 \\
\hline
极小值 & $0$ & $+1$ & 连通分量出生 \\
一阶鞍点 & $1$ & $-1$ & 亏格 $+1$（生成通道/环）\\
二阶鞍点 & $2$ & $+1$ & 亏格 $-1$（通道闭合）\\
极大值 & $3$ & $-1$ & 连通分量消亡 \\
\hline
\end{tabular}
...
% ---- tree with a saddle: two lower edges merge at saddle, one upper edge ----
\node[crit node] (L1) at (-0.9, 6) {};
\node[crit node] (L2) at ( 0.9, 6) {};
\node[sad node]  (S)  at ( 0.0, 3) {$S$};
\node[crit node] (R)  at ( 0.0, 0) {};
\draw[main edge] (L1) -- (S);
\draw[main edge] (L2) -- (S);
\draw[main edge] (S)  -- (R);
\node[lab, align=center] at (0,-1.6) {跨鞍点 $S$};

% level below saddle: two separate components
\draw[lev] (0,1.5) -- (2.0,1.5);
% level above saddle: one component
\draw[lev] (0,4.5) -- (2.0,4.5);

% ---- below saddle: two spheres (2 components, g=0) ----
\begin{scope}[shift={(3.1,1.5)}]
  \draw[iso] (-0.35,0) circle (0.28);
  \draw[iso] ( 0.35,0) circle (0.28);
\end{scope}
\node[lab, align=center, text=green!35!black] at (3.1,-1.6) {鞍点下：两分量\\$c\!=\!2,\ g\!=\!0$};

% ---- above saddle: one torus (merged, genus 1) ----
\begin{scope}[shift={(3.1,4.5)}]
  \draw[iso] (0,0) ellipse (0.6 and 0.34);
  \draw[iso] (0,0) ellipse (0.24 and 0.13);
\end{scope}
\node[lab, align=center, text=green!35!black] at (3.1,6.3) {鞍点上：合并\\$c\!=\!1,\ g\!=\!1$};
\end{tikzpicture}
\caption{阈值穿过结构节点时拓扑属性跳变。跨越鞍点 $S$ 前后，等值面由两个独立分量（$c=2$，$g=0$）经通道合并/连通性重组为单个更复杂的曲面（$c=1$，$g=1$）；map 在结构节点处切换，故结构节点必须保留。}
