...
void printUsage() {
  std::cerr
    << "Usage: treeTxtIsmSimplifier -t <tree.txt> -s <field.vti> "
...
vtkIdType ijkToId(const std::array<int, 3> &ijk, int dimX, int dimY) {
  return static_cast<vtkIdType>(ijk[2]) * dimY * dimX
         + static_cast<vtkIdType>(ijk[1]) * dimX
         + ijk[0];
}

struct LocalBinaryBlock {
  std::array<int, 6> extent{};
  int dimX{};
  int dimY{};
  int dimZ{};
...
LocalBinaryBlock extractLocalBlock(const NodeRecord &node,
                                   vtkImageData *image,
  ...
  std::unordered_set<int> regionIds(node.subtreeVertices.begin(),
                                    node.subtreeVertices.end());
  ...
  LocalBinaryBlock block{};
  block.extent = {minCoord[0], maxCoord[0], minCoord[1], maxCoord[1], minCoord[2], maxCoord[2]};
  block.dimX = maxCoord[0] - minCoord[0] + 1;
  block.dimY = maxCoord[1] - minCoord[1] + 1;
  block.dimZ = maxCoord[2] - minCoord[2] + 1;

  const size_t voxelCount
    = static_cast<size_t>(block.dimX) * block.dimY * block.dimZ;
  block.insideMask.assign(voxelCount, 0);
  block.scalarValues.assign(voxelCount, 0.0f);
  ...
  for(int z = 0; z < block.dimZ; ++z) {
    for(int y = 0; y < block.dimY; ++y) {
      for(int x = 0; x < block.dimX; ++x) {
        ...
        const vtkIdType globalId = ijkToId(global, dims[0], dims[1]);
        const vtkIdType localId
          = static_cast<vtkIdType>(z) * block.dimY * block.dimX
            + static_cast<vtkIdType>(y) * block.dimX
            + x;
        block.insideMask[localId]
          = regionIds.count(static_cast<int>(globalId)) > 0 ? 1 : 0;
        block.scalarValues[localId]
          = static_cast<float>(scalarArray->GetComponent(globalId, 0));
      }
    }
  }
...
std::vector<unsigned char> buildBoundaryMask(const LocalBinaryBlock &block) {
  std::vector<unsigned char> boundaryMask(block.insideMask.size(), 0);
      ...
      for(int x = 0; x < block.dimX; ++x) {
        ...
        for(int d = 0; d < 6; ++d) {
          ...
          if(nx < 0 || nx >= block.dimX
             || ny < 0 || ny >= block.dimY
             || nz < 0 || nz >= block.dimZ) {
            isBoundary = block.insideMask[idx] != 0;
            if(isBoundary) {
              break;
            }
            continue;
          ...
          if(block.insideMask[nidx] != block.insideMask[idx]) {
            isBoundary = true;
            break;
          }
        }
...
std::vector<float> buildDistanceField(const LocalBinaryBlock &block,
                                      const std::vector<unsigned char> &boundaryMask) {
  std::vector<float> distanceField(boundaryMask.size(),
...
std::array<double, 3> computeCentroid(const LocalBinaryBlock &block,
                                      const std::vector<unsigned char> &boundaryMask) {
  double sumX = 0.0;
  double sumY = 0.0;
  double sumZ = 0.0;
  ...
  for(int z = 0; z < block.dimZ; ++z) {
    for(int y = 0; y < block.dimY; ++y) {
      for(int x = 0; x < block.dimX; ++x) {
        ...
        sumX += block.extent[0] + x;
        sumY += block.extent[2] + y;
        sumZ += block.extent[4] + z;
        ++count;
      }
    }
  }
...
double computeBboxDiagonal(const std::array<int, 6> &extent) {
  const double dx = extent[1] - extent[0];
  const double dy = extent[3] - extent[2];
  const double dz = extent[5] - extent[4];
  return std::sqrt(dx * dx + dy * dy + dz * dz);
}

IsmDescriptor buildDescriptor(const NodeRecord &node,
                              vtkImageData *image,
                              vtkDataArray *scalarArray) {
  const auto block = extractLocalBlock(node, image, scalarArray);
  IsmDescriptor desc{};
  desc.nodeId = node.nodeId;
  desc.extent = block.extent;
  desc.boundaryMask = buildBoundaryMask(block);
  desc.distanceField = buildDistanceField(block, desc.boundaryMask);
  desc.centroid = computeCentroid(block, desc.boundaryMask);
...
double computeDistISM(const IsmDescriptor &descA,
...
double computeCentroidDistance(const IsmDescriptor &a, const IsmDescriptor &b) {
  double sum = 0.0;
  for(int d = 0; d < 3; ++d) {
...
double computeBboxRelativeDifference(const IsmDescriptor &a, const IsmDescriptor &b) {
...
SimplificationResult simplifyBranches(const std::vector<BranchRecord> &branches,
  ...
  std::unordered_map<int, IsmDescriptor> descriptorCache{};
  auto getDescriptor = [&](int nodeId) -> const IsmDescriptor & {
...
buildSimplifiedNodeGrid(const SimplifiedTree &tree,
  ...
  for(const auto nodeId : tree.keptNodeIds) {
    ...
    subtreeSizeArray->InsertNextValue(static_cast<int>(node.subtreeVertices.size()));
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cout << "ISM simplification finished. Kept "
